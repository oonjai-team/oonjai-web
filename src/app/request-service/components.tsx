...
export const Header = () => (
  <header className="flex items-center justify-between px-5 md:px-8 py-4 bg-[#F5EEDC] backdrop-blur-md border-b border-gray-200 text-[#3A5A40] sticky top-0 z-[1000]">
    ...
    <nav className="hidden md:flex gap-8 text-sm font-medium">
      <a href="#" className="group relative flex flex-col items-center hover:text-teal-900 transition-all">
        <div className="flex items-center gap-2 group-hover:font-bold"><BookOpen size={16} /> Activity Marketplace</div>
        <span className="block font-bold h-0 overflow-hidden invisible pointer-events-none" aria-hidden="true">
          <span className="inline-flex gap-2"><BookOpen size={16} /> Activity Marketplace</span>
        </span>
      </a>

      <a href="/request-service" className="flex items-center gap-2 text-teal-800 font-bold border-b-2 border-teal-800 pb-1">
        <ClipboardList size={16} /> Request a Service
      </a>

      <a href="#" className="group relative flex flex-col items-center hover:text-teal-900 transition-all">
        <div className="flex items-center gap-2 group-hover:font-bold"><BookOpen size={16} /> My Status</div>
        <span className="block font-bold h-0 overflow-hidden invisible pointer-events-none" aria-hidden="true">
          <span className="inline-flex gap-2"><BookOpen size={16} /> My Status</span>
        </span>
      </a>

      <a href="#" className="group relative flex flex-col items-center hover:text-teal-900 transition-all">
        <div className="flex items-center gap-2 group-hover:font-bold"><Settings size={16} /> Settings</div>
        <span className="block font-bold h-0 overflow-hidden invisible pointer-events-none" aria-hidden="true">
          <span className="inline-flex gap-2"><Settings size={16} /> Settings</span>
        </span>
      </a>
    </nav>

    <button className="hidden md:flex group relative flex-col items-center text-sm font-medium transition-all">
      <div className="flex items-center gap-2 group-hover:font-bold">
        <User size={16} /> <span>Account</span> <ChevronLeft size={16} className="rotate-180" />
      </div>
      <span className="block font-bold h-0 overflow-hidden invisible pointer-events-none" aria-hidden="true">
        <span className="inline-flex items-center gap-2"><User size={16} /> Account <ChevronLeft size={16} className="rotate-180" /></span>
      </span>
    </button>
